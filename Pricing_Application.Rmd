...
```
and we can use the effects to model te density of a win *(ResultEffect[2])*, comparing green RSFEffect[1] customers with platinum customers RSFEffect[4]. The value of RSF is evident here as platinum customers 
...
p10 <- ggplot(quoteData) + 
  geom_line(aes(QuoteDiff,
                y= dnorm(QuoteDiff, 
                         mean = dMn+RSFEffect[1]+ResultEffect[2]+(ATPEffect*ATPDiff), 
                         sd = sigma[1])), color  = 'darkgreen') +
  geom_line(aes(QuoteDiff,y= dnorm(QuoteDiff, mean = dMn+RSFEffect[4]+ResultEffect[2]+(ATPEffect*ATPDiff), 
